...
function AddressValidationModal({ formState, onValidationResult }) {
  ...
  const handleProceedWithoutValidation = () => {
    const addressWithoutValidation = `${formState.recipientHouseNumber}, ${formState.recipientPostalZone}, ${formState.recipientCity}`;

    const data = {
      textForm: addressWithoutValidation,
      addressId: null,
      lat: null,
      lng: null,
    };
    onValidationResult(data, null);
    handleClose();
    setSelectedCheckboxData(null);
  };
                      ...
                      <div className="sm:col-span-1 xs:col-span-1">
                        {member.customerId !== "unreg" ? (
                          <FormCheck
                            ...
                            id={`checkbox-${index}`}
                            label=""
                            checked={selectedCheckboxData === member.customerId}
                            onChange={() =>
